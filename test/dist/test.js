...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrmrmse, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an accumulator function', function test( t ) {
	t.equal( typeof incrmrmse( 3 ), 'function', 'returns a function' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current root mean squared error', function test( t ) {
	...
	t.equal( acc(), sqrt( 145.0/2.0 ), 'returns expected value' );
	t.end();
});
